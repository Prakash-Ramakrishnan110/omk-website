  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      await supabase.from('audit_logs').insert([{
        admin_email: 'admin@omktamilnadu.org', // Fallback, normally would come from auth
        action_type: 'MEMBER_ADDED_BY_ADMIN',
        description: `Admin manually added and approved member: ${data.fullName}`,
        target_id: insertedMember.id
      }]);

      // 5. Redirect to Member Details Page
      navigate(`/admin/members/${insertedMember.id}`);
